      SUBROUTINE get_bspline_coeffs2(numatoms,fr1,fr2,fr3,order,theta1
     &     ,theta2,theta3,dtheta1,dtheta2,dtheta3,d2theta1,d2theta2
     &     ,d2theta3,nfft1,nfft2,nfft3,kstart,kend,jstart,jend,indk1
     &     ,indk2,indj1,indj2,mk,mj)

************************************************************************
*   Time-stamp: <2005-02-03 17:21:06 marchi>                             *
*                                                                      *
c---------------------------------------------------------------------
c INPUT:
...
*----------------------------- ARGUMENTS ------------------------------*
     ...
     &     ,d2theta1(order,*),d2theta2(order,*),d2theta3(order,*)
...
*----------------------- EXECUTABLE STATEMENTS ------------------------*
      ...
      DO n = 1,numatoms
         ...
         IF(mkk .NE. 0) THEN
            ...
            IF(mjj .NE. 0) THEN
               w = fr1(n)-int(fr1(n))
               call fill_bspline2(w,order,theta1(1,n),dtheta1(1,n)
     &              ,d2theta1(1,n))
               w = fr2(n)-int(fr2(n))
               call fill_bspline2(w,order,theta2(1,n),dtheta2(1,n)
     &              ,d2theta2(1,n))
               w = fr3(n)-int(fr3(n))
               call fill_bspline2(w,order,theta3(1,n),dtheta3(1,n)
     &              ,d2theta3(1,n))
            END IF
         END IF
      END DO
