      SUBROUTINE tors(kprint,knlist,jerr,alpha,n1,nalpha,iret,iline
     &     ,errmsg)
...
100   READ(knlist,'(a78)') line(1:78)
	  ...
	  m=nword/4
	  IF(m*4.EQ.nword) THEN
	      DO 10 i=1,nword,4
		  n=n+1
                  alpha(1,n)=strngs(i)(1:7)
                  alpha(2,n)=strngs(i+1)(1:7)
                  alpha(3,n)=strngs(i+2)(1:7)
                  alpha(4,n)=strngs(i+3)(1:7)
10            CONTINUE
          ELSE
	      iret=1
	      errmsg=' In TORS: Needs four atoms to define a torsion '
	      RETURN
          END IF
	  GOTO 100
      END IF
