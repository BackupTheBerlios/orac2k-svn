...
!!$***********************************************************************
!!$   Time-stamp: <04/12/15 14:14:53 marchi>                           *
!!$                                                                      *
!!$                                                                      *
!!$                                                                      *
!!$======================================================================*
...
!!$------------------------- LOCAL VARIABLES ----------------------------*
    ...
    Gives_Dim=SUM(Takes(1:nprocs)%natoms)*3
    ALLOCATE(Rbuff_r(Takes_Dim*3),Rbuff_s(Gives_Dim*3))
    mp=Takes_Dim+Gives_Dim
    CALL Buffer_Attach(mp)
    ...
    DO k=1,exchg
       ...
       IF(node == dest) THEN
          ...
          CALL MPI_RECV(Rbuff_r,mp3,MPI_REAL8,source,0,MPI_COMM_WORLD&
               &, istatus ,ierr)
          WRITE(50+node,'(3i8)') dest,source,mp
          DO ii=1,mp
...
!!$------------------------------------------------------------------------
  ...
  SUBROUTINE Setup_Comm
    IMPLICIT NONE 
    INTEGER :: i,j,count,nst,ned,nst_m,ned_m,inode,jj,m
  ...
  SUBROUTINE Pass_Takes_Gives
    ...
    INTEGER :: ierr,mp,k,dest,source,istatus(MPI_STATUS_SIZE)
...
!!$
    ...
    DO k=1,exchg
       ...
       mp=0
       IF(node == dest) THEN
          CALL MPI_RECV(mp,1,MPI_INTEGER,source,0,MPI_COMM_WORLD&
               &, istatus ,ierr)
          CALL MPI_RECV(Ibuff_r,mp,MPI_INTEGER,source,1,MPI_COMM_WORLD&
               &, istatus ,ierr)
       ...
       ELSE IF(node == source) THEN
          ...
          END DO
          CALL MPI_BSEND(mp,1,MPI_INTEGER,dest,0,MPI_COMM_WORLD,ierr)
          CALL MPI_BSEND(Ibuff_s,mp,MPI_INTEGER,dest,1,MPI_COMM_WORLD,ierr)
       END IF
    END DO
    CALL Buffer_Detach
...
!!$
  ...
  SUBROUTINE Buffer_Attach(m)
    IMPLICIT NONE 
    INTEGER :: m
    INTEGER :: ierr

    CALL MPI_PACK_SIZE(m,MPI_REAL8,MPI_COMM_WORLD,Buffer_p&
         &,ierr)
