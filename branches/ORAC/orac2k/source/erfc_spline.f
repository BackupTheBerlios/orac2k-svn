     ...
     $       erf_arr,tau,alpha,kcut,lerf)

      implicit none
      integer mxerftab
      double precision erftbdns,erf_arr(4,*),tau(*),kcut
      logical lerf

      double precision del,x,pi,fac,erf,erft
      integer i,ibcbeg,ibcend
      REAL*8 a1,a2,a3,a4,a5,qp,qt,expcst,erfcst,func,kcut_max,s,work
     &     ,alpha
      external func
      common /for_func/work,x
      ...
      do 100 i = 1,mxerftab
        x = del*(i-1)
        if(lerf) call QSIMP(FUNC,kcut,kcut_max,S) 
        qt=1.0D0/(1.0D0+qp*x)
        expcst=DEXP(-x*x)
        erf=((((a5*qt+a4)*qt+a3)*qt+a2)*qt+a1)*qt*expcst
        tau(i) = x
        erf_arr(1,i) = erf + 2.d0*s/pi
100   continue
      ibcbeg = 1
      ibcend = 1
